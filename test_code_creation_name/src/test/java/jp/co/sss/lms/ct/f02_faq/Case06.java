...
public class Case06 {
	...
	void test02() throws Exception {
		...
		getEvidence("Case06/02-1.Login_Before");
		...
		getEvidence("Case06/02-2.Login_Success");
	...
	void test05() throws Exception {
		// ConfigUtilsからカテゴリIDを取得
		String categoryId = ConfigUtils.getFaqCategoryId();

		// カテゴリリンクをクリック
		WebDriverUtils.webDriver.findElement(
				By.cssSelector("ul li a[href*='frequentlyAskedQuestionCategoryId=" + categoryId + "']")).click();
		Thread.sleep(2000);

		getEvidence("Case06/05-1.Category_Result");

		// 成功判定
		String currentUrl = WebDriverUtils.webDriver.getCurrentUrl();
		assertTrue(currentUrl.contains("frequentlyAskedQuestionCategoryId=" + categoryId),
				"検索結果のURLに選択したカテゴリID（" + categoryId + "）が含まれていません。");
	}
	...
	void test06() throws Exception {
		// 検索結果の最初の質問の <dt> を取得
		By firstQuestionSelector = By.cssSelector("dl dt");
		WebDriverUtils.webDriver.findElement(firstQuestionSelector).click();
		Thread.sleep(500); // アニメーションや表示切替の待機

		// クリックした質問の回答 <dd> を取得
		By firstAnswerSelector = By.cssSelector("dl dd");
		String answerText = WebDriverUtils.webDriver.findElement(firstAnswerSelector).getText();

		getEvidence("Case06/06-1.Answer_Display");

		// 成功判定
		assertFalse(answerText.isEmpty(), "回答が表示されていません");
	}
